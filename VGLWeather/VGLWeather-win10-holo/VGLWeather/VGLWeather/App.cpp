...
void App::SetWindow(CoreWindow^ window)
{
  ...
  try
  {
    ...
    mLocator = SpatialLocator::GetDefault();
    mStationaryReferenceFrame = mLocator->CreateStationaryFrameOfReferenceAtCurrentLocation();

    mLocator->LocatabilityChanged +=
      ref new TypedEventHandler<SpatialLocator^, Object^>(this, &App::OnLocatabilityChanged);
...
void App::RecreateRenderer()
{
  ...
  if(!mRenderer)
  {
    ...
    {
      mRenderer.reset(new Renderer(true, 1.0f, 0));
      mRenderer->setReferenceFrame(mStationaryReferenceFrame);
    }
    else
    {
      mRenderer.reset(new Renderer(false, 1.0f, 0));
    }
  }
}
...
void App::makeContextCurrent()
{
  ...
  {
    throw Exception::CreateException(E_FAIL, L"Failed to make fullscreen context current");
  }
}
...
void App::Run()
{
  while(!mWindowClosed)
  {
    if(mWindowVisible)
    {
      ...
      if(!mHolographicSpace || firstRender)
      {
        EGLint panelWidth = 0;
      ...
      {
        verr << "Detected D3D Device Lost!!!" << std::endl;
...
App ^App::getCurrent()
{
  return currentInstance.Resolve<App>();
}
...
void App::OnVisibilityChanged(CoreWindow^ sender, VisibilityChangedEventArgs^ args)
{
  ...
  if(mRenderer)
  {
    mRenderer->setAppSuspended(!mWindowVisible);
  }
}
...
void App::OnLocatabilityChanged(Windows::Perception::Spatial::SpatialLocator ^locator, Platform::Object ^args)
{
  if(!mRenderer)
    return;

  vgl::System::system().scheduleTask([locator, this] {
    switch(locator->Locatability)
    {
    ...
    {
      auto limitedRefFrame = mLocator->CreateAttachedFrameOfReferenceAtCurrentHeading();

      eglExtSurfaceAttribPointerANGLE(mEglDisplay, mEglSurface, EGLEXT_HOLOGRAPHIC_SPATIAL_FRAME_OF_REFERENCE_ANGLE,
        reinterpret_cast<IInspectable*>(limitedRefFrame));

      //mRenderer->setTrackingLossState(true, limitedRefFrame);
    }
    break;
    case SpatialLocatability::PositionalTrackingActive:
    {
      eglExtSurfaceAttribPointerANGLE(mEglDisplay, mEglSurface, EGLEXT_HOLOGRAPHIC_SPATIAL_FRAME_OF_REFERENCE_ANGLE,
        reinterpret_cast<IInspectable*>(mStationaryReferenceFrame));
      //mRenderer->setTrackingLossState(false);
    }
    break;
    }
  });
